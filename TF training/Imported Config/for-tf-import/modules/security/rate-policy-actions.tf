...
resource "akamai_appsec_rate_policy_action" "tfdemo_post_and_put_page_requests" {
  config_id          = local.config_id
  security_policy_id = akamai_appsec_security_policy.tfdemo.security_policy_id
  rate_policy_id     = akamai_appsec_rate_policy.post_and_put_page_requests.rate_policy_id
  ipv4_action        = "alert"
  ipv6_action        = "alert"
}
